...
- 工作区选择在 Tauri 桌面版统一使用系统原生目录对话框（`tauri-plugin-dialog` 的 `open({ directory: true })`），并记住最近一次选择的路径作为下一次对话框默认定位（localStorage `last_project_dir`）；浏览器预览版保留 `webkitdirectory` 目录上传控件作为兜底。Tauri 原生选择返回本地绝对路径，由后端 `DirectorySource` 复制进隔离工作区，配额与浏览器上传一致（10000 文件、单文件 10MB、总文本 100MB）。
...
- “岗位准备”是独立主页面：一个面试者可维护多个目标岗位，每个岗位可关联多个项目并拥有独立题库；由岗位题目发起的练习会话必须保留岗位和题目关联。
- “简历库”采用同屏两栏 master-detail：左栏简历列表（顶部 CANDIDATE CONTEXT + 上传简历按钮 + 搜索框 + 列表），右栏展示选中简历详情；删除操作保留在详情页头部。列表项右侧带编辑/删除按钮（编辑弹窗可改姓名/岗位/领域并可选替换 PDF，替换后主张按新文本重新提取、skip 设置重置）；列表支持拖拽排序（搜索过滤时禁用）并持久化到后端；左栏宽度可拖拽调整（240–480px，localStorage 记忆），分隔条支持键盘方向键和双击恢复默认宽度。
...
新建复盘会话页支持选择单 Agent 或多 Agent 角色分工：单 Agent 选择一个 Agent 负责全部阶段；多 Agent 分工为出题、评价、追问策略三个阶段分别选择 Agent。Agent 列表来自 `GET /settings/agents`（内置 + 用户自定义），内置角色带「内置」标记。应用设置页的 Agent 管理支持自定义 Agent 的创建、编辑和删除：自定义内容包括名称、角色、persona 角色设定和绑定的 LLM 配置档案（不绑定则跟随当前激活配置）。persona 只影响角色描述，JSON 输出契约由后端固定，表单中需向用户说明这一点。会话的 Agent 组合随会话持久化，会话页的默认选择是单 Agent 全能面试官。
## 当前原型设计决策
...
- 证据面板在“当前问题无证据/无评价”时展示引导空状态（图标 + 说明 + 「前往项目资料」入口），不使用纯空白占位；工作台内消息类容器圆角统一为 12px 系列（问题卡 12px、历史卡 10px、系统消息 12px、输入区 12px），证据来源等弱对比文字统一走 `--muted` token。
